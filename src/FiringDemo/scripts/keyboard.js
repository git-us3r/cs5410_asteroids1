...
// A keyboard object. 
var Keyboard = function(thrustKeyCode, fireKeyCode){
	...
	that.keyReleased = function( event ){
		...
		if (event.keyCode === thrustKeyCode || event.keyCode === fireKeyCode){
		
			that.keys[event.keyCode] = -1;
		}
		else{

			delete that.keys[event.keyCode];
		}
		//console.log(that.keys[event.keyCode]);
	...
	that.update = function(elapsedTime){
		...
		for (key = 0; key < that.handlers.length; key++) {
				...
				var code = that.keys[that.handlers[key].key];
				that.handlers[key].handler(elapsedTime, code);
				//console.log(code);
	...
	// a function (a member of that), which is associated with that key.
		//
	// ------------------------------------------------------------------
	that.registerKey = function(_key, _handler){

		// Add the new pair.
		that.handlers.push({key: _key, handler: _handler});
